...
if ($credits>=$i_price)
{
	...
	->messageHtml(file_get_contents("../email/template_top.txt")."Spark Lemon Item Purchase: ".$i_name.file_get_contents("../email/template_middle.txt").'<p><b>Hello '.$f_name.',</b></p>
<p>You have purchased "'.$i_name.'" item for $'.$i_price.'.  You can view and download all your purchases on your Spark Lemon account.</p>
'.$prefs_email_html_signature.file_get_contents("../email/template_bottom.txt"))
	...
	$date=time();
	mysql_query("INSERT INTO purchases (user_from,user_to,amount,item_id,date,downloads) VALUES ('$user_id','$i_seller','$i_price','$i_id','$date','0')");
