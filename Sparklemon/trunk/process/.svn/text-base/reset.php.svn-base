...
->messageHtml(file_get_contents("../email/template_top.txt")."Spark Lemon Password Reset".file_get_contents("../email/template_middle.txt").'<p><b>Hello '.$name.',</b></p>
<p>A password reset was requested for your account. If you did not request one, please ignore this email. To reset your password, please click on the following link.</p>
<p><a href="'.$prefs_sitebase.'/confirm.php?token='.urlencode($token).'&email='.urlencode($email).'&id=1">'.$prefs_sitebase.'/confirm.php?token='.urlencode($token).'&email='.urlencode($email).'&id=1</a></p>
