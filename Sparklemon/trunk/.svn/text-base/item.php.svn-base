...
if ($i_hidden == 1 && $is_admin == 0)
{
	echo "This file is not available at the moment.";
	require('footer.php');
	exit;
}
?>
...
$show_subtype = get_item_type($i_type,$i_subtype);
...
if ($credits>=$i_price) {
	...
	{
		$buy_string="Please login to buy an item.  You can also manage your downloads once logged in.";
	}
}
?>

</li><li><?php echo $show_subtype; ?></li></ol>
<p>
...
<div class="sidebar">
    <div class="user">
        ...
        <span class="badges"><?php /*{ badges go here }*/ get_badges($i_seller); ?></span>
    </div>
	<?php if (mysql_num_rows(mysql_query("SELECT * FROM purchases WHERE user_from='$user_id' AND item_id='$resource_id'"))==0) : ?>
	<div class="buybox">
		<a id="<?php if ($user_id != $i_seller) { echo 'buypress'; } else { echo 'nopress'; }?>" href="<? if ($user_id == $i_seller) { echo $prefs_sitebase.'/process/download.php?id='.$resource_id; } else { echo '#'; } ?>" style="text-decoration:none;">
		<div <?php if (/*$user_id != $i_seller*/true) {echo 'class="button buy"';} ?>>
			<?php if ($user_id != $i_seller) : ?> BUY {<span class="price">$<?php echo $i_price; ?></span>}<?php
            endif;
            if ($user_id == $i_seller) : ?>
            	you own this - {<span class="price">$<?php echo $i_price; ?></span>}
            <?php endif; ?>
        </div>
        </a>
		<?php
    ...
    <p style="margin-left: 15px;"><strong>Sales:</strong> <?php echo $i_sales; ?><br /><strong>Date Added:</strong> <?php echo date("jS F Y",$i_date); ?>
	<?php if ($i_tags != '') : ?><br /><strong>Tags:</strong> 
	<?php 
		$individual_tag = explode(" ", $i_tags);
		foreach ($individual_tag as $tag) {
			echo '<a href="'.$prefs_sitebase.'/browse/tag/'.$tag.'"><span class="tag">'.$tag.'</span></a>';
		}
	endif;
	?></p>
