...
if(isset($_GET['add_id'])){
  $query = mysql_query("INSERT INTO `enrolled` (`user_id`,`assignment_id`) VALUES ($user_id, ".mysql_real_escape_string($_GET['add_id']).")") ;
}
  $query = mysql_query("SELECT `assignments`.`id`,`assignments`.`title`,`assignments`.`desc`,`assignments`.`difficulty` FROM `assignments`,`enrolled` WHERE `enrolled`.`grade` != '-1' AND `enrolled`.`user_id` = $user_id AND `assignments`.`id` = `enrolled`.`assignment_id` ORDER BY `assignments`.`difficulty`");
      ...
      <div class="span12">
        <div class="border-solid box-padding step-container"><span class="step"><?=$x;?></span>
          ...
          <p><?=$row['desc'];?></p>
          <form class="form-horizontal" method="post" action="my-submissions.php?ass_id=<?=$row['id'];?>">
            <div class="form-actions-in">
              <button type="submit" class="btn btn-large btn-info">View Score</button>
            </div>
          </form>
        </div>
      </div>
    ...
    <div class="space"></div>
        ...
        $x++;
           ...
           <div class="alert alert-error"><a href="#" class="close" data-dismiss="alert">&times;</a>
              <p>It appears you haven't completed any assignments yet, or we haven't gotten around to marking it yet.</p>
           </div>
        <?php
          }
          ?>
